// Auto-generated feature route: athlete-pipeline
// Domain: competitive athlete pipeline coach — Capability: producing specialized training tracks
// Mounted under: /api/athlete-pipeline
...
// POST /api/athlete-pipeline/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a competitive athlete pipeline coach. Your task is producing specialized training tracks. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/athlete-pipeline/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/athlete-pipeline/:id — fetch one
...
// DELETE /api/athlete-pipeline/:id — discard a run
