// Auto-generated feature route: belt-advisor-agent
// Domain: belt progression advisor — Capability: predicting belt test success and giving real-time feedback
// Mounted under: /api/belt-advisor-agent
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Title': 'Martial Art Dojo Manager'
      }
    };
...
// POST /api/belt-advisor-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a belt progression advisor. Your task is predicting belt test success and giving real-time feedback. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/belt-advisor-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/belt-advisor-agent/:id — fetch one
...
// DELETE /api/belt-advisor-agent/:id — discard a run
