// Auto-generated feature route: tournament-organizer
// Domain: autonomous tournament organizer — Capability: producing brackets, scheduling, and live scoring plans
// Mounted under: /api/tournament-organizer
...
// POST /api/tournament-organizer/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a autonomous tournament organizer. Your task is producing brackets, scheduling, and live scoring plans. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/tournament-organizer/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/tournament-organizer/:id — fetch one
...
// DELETE /api/tournament-organizer/:id — discard a run
