...
// Route: /api/gap-payment
// Feature: Payment
// Description: Payment processor integration (recurring billing)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Payment. Payment processor integration (recurring billing)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'payment', title: 'Payment', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'payment', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['payment']);
      return res.json({ slug: 'payment', items: r.rows || [] });
    }
    return res.json({ slug: 'payment', items: [] });
  } catch (e) {
    return res.json({ slug: 'payment', items: [], error: (e && e.message) || 'Server error' });
  }
});
