...
// Route: /api/gap-student-performance-analytics
// Feature: Student Performance Analytics
// Description: (struggling-student detection)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Student Performance Analytics. (struggling-student detection)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Student Performance Analytics\nDescription: (struggling-student detection)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['student-performance-analytics', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'student-performance-analytics', title: 'Student Performance Analytics', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'student-performance-analytics', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['student-performance-analytics']);
      return res.json({ slug: 'student-performance-analytics', items: r.rows || [] });
    }
    return res.json({ slug: 'student-performance-analytics', items: [] });
  } catch (e) {
    return res.json({ slug: 'student-performance-analytics', items: [], error: (e && e.message) || 'Server error' });
  }
});
