...
// Route: /api/gap-equipment-demand-forecast
// Feature: Equipment Demand Forecast
// Description: (uniform/gear forecasting)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Equipment Demand Forecast. (uniform/gear forecasting)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Equipment Demand Forecast\nDescription: (uniform/gear forecasting)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['equipment-demand-forecast', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'equipment-demand-forecast', title: 'Equipment Demand Forecast', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'equipment-demand-forecast', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['equipment-demand-forecast']);
      return res.json({ slug: 'equipment-demand-forecast', items: r.rows || [] });
    }
    return res.json({ slug: 'equipment-demand-forecast', items: [] });
  } catch (e) {
    return res.json({ slug: 'equipment-demand-forecast', items: [], error: (e && e.message) || 'Server error' });
  }
});
