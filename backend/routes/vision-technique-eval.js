// Auto-generated feature route: vision-technique-eval
// Domain: martial arts technique evaluator — Capability: scoring technique from uploaded videos
// Mounted under: /api/vision-technique-eval
...
// POST /api/vision-technique-eval/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a martial arts technique evaluator. Your task is scoring technique from uploaded videos. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vision-technique-eval/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vision-technique-eval/:id — fetch one
...
// DELETE /api/vision-technique-eval/:id — discard a run
